...
- [ ] [H1] 🐱 고양이를 보고 기겁하며 도망치는 크리퍼 상성 고증 (Creeper Fears Cats)
  - 고양이 펫 소환 시 크리퍼가 반경 내로 접근하면 도화선을 즉시 멈추고 "하악질! 😾" 소리와 함께 반대 방향으로 패닉 도주
- [ ] [H2] 🛡️ 왼손 방패 가드 (Off-hand Shield Guard)
  - 왼손에 3D 마인크래프트 방패 장착 및 웅크리기(Shift 연타) 시 방패를 번쩍 들어 올려 크리퍼 폭발 및 공격을 `챙-!` 소리와 함께 100% 방어
- [ ] [H3] 👁️ 엔더맨(Enderman) 출현 및 시선 마주침(Staring Contest) 인터랙션
  - 평소에는 블록을 들고 배회하다가 마우스 커서(시선)가 머리를 마주치는 순간 입을 벌리고 괴성을 지르며 격노 상태로 순간이동 돌진하는 AI 구현
