...
### 2026-09-11
- **[앵무새 꼬리 공중 분리 및 각도 수정] 관절 피벗 및 메쉬 오프셋·경사각 교정**: 앵무새(`Parrot`) 펫의 긴 꼬리 깃털이 몸통에서 0.16 이상 멀리 떨어진 허공에 붕 떠 있고 몸 안쪽을 향하던 결함 수정.
  - **원인 분석**: 꼬리 메쉬(`tailMesh`)에 로컬 위치 `(0, -0.50, -0.16)`가 중복 지정된 상태에서 관절(`tailJoint`) 위치가 `(0, 0.265, -0.160)`으로 잡혀, Z축으로 뒷면(`-0.16`) 바깥인 `-0.32`에 배치되고 Y축으로도 피벗과 메쉬 중심이 어긋나 흔들기 애니메이션(`wag`) 시 꼬리가 허공에서 거대한 호를 그리며 궤도를 도는 결함 발생. 또한 피치 회전 부호가 음수(`-0.18`)여서 꼬리 끝이 몸통 안쪽(앞쪽 `|/`)으로 파고드는 현상 확인.
  - **수정 내용**: 관절 피벗을 몸통 뒷면 하단 `tailJoint.position = (0, -0.08, -0.15)`에 매립하고, 메쉬는 피벗 기준 상단 뿌리가 회전축이 되도록 `tailMesh.position = (0, -0.24, 0)` 및 양수 피치 `tailMesh.eulerAngles.x = 0.22`로 조정하여 원작처럼 몸통 뒤쪽 바깥으로 비스듬히(`|\`) 뻗어나가도록 교정.
  - **검증**: ① SceneKit 다각도 렌더링(측면, 후면 3/4 쿼터뷰, 전면 3/4 쿼터뷰, 대기 중 wagging, 걷기/비행 날개짓, 앉기) 픽셀 및 시각적 검증 통과(단절 간격 0px, 외향 각도 `|\` 확인), ② 전 펫 5종 모델 렌더링 회귀 테스트 통과, ③ `build_app.sh` 번들 패키징 및 스모크 테스트 정상 통과.
